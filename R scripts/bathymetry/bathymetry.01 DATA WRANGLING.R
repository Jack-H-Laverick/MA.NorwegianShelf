...
# Clip longitudes
H_crop <- which(between(nc_lon, 1, 21)); W <- min(H_crop) ; E <- max(H_crop) # Find max and min index in target window
nc_lon <-nc_lon[c(W:E)]                                         # Clip to window

# Clip latitudes
V_crop <- which(between(nc_lat, 58, 72)); S <- min(V_crop); N <- max(V_crop)  # Find max and min index in target window
