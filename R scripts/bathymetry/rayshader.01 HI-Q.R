...
S <- nrow(nc_lat)*(151/180) ; N <- nrow(nc_lat)*(159/180)
W <- length(nc_lon)*177/360 ; E <- length(nc_lon)*202/360     # For Mercatore
...
mat %>%
  ...
  plot_3d(mat, zscale = 20, fov = 0, theta = 60, phi = 30, 
          windowsize = c(1280, 640), zoom = 0.33,
...
render_highquality(print_scene_info = T, parallel = TRUE, lightintensity = 0, samples = 150, filename = "rayshade-hi.png", 
                   scene_elements = bind_rows(rayrender::sphere(z = 100, y = 13000, x = 600, radius = 500,
                                              ...
                                              rayrender::text3d(label = "Norwegian Shelf", angle = c(30, 60, 0), z = 5000, y = 1600, x = -650, 
