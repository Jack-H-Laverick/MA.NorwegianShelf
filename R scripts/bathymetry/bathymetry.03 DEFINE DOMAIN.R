...
Bathymetry <- st_as_sf(Bathymetry)  %>%           # Convert the stars grid into SF polygons which can be merged
  st_transform(crs = crs) %>% 
  st_join(dplyr::select(close, -Elevation)) %>%                             # Attach distance from shore for simultaneous filtering
  drop_na()

Domains <- mutate(Bathymetry, Shore = ifelse(between(Elevation, -600, -60) & Shore_dist > 20000, "Offshore",
